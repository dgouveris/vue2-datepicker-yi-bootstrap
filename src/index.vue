  ...
  >
    <div class="mx-input-wrapper" @click.stop="showPopup">
      <input
        ...
        @change="handleChange"
      />
      <span v-if="showClearIcon" class="mx-input-append mx-clear-wrapper" @click.stop="clearDate">
        ...
        <slot name="calendar-icon">
          ...
          >
            <g>
              <g>
                <path
                  d="M 19.237 17.926 L 113.768 17.926 C 121.5 17.926 127.768 24.194 127.768 31.926 L 127.768 82.032 M 72.083 129.19 L 19.237 129.19 C 11.505 129.19 5.237 122.922 5.237 115.19 L 5.237 31.926 C 5.237 24.194 11.505 17.926 19.237 17.926"
                  fill="transparent"
                />
                <line x1="28.176184" x2="28.176184" y1="2.966054" y2="32.542563" />
                <line x1="4.937499" x2="127.46875" y1="46.626615" y2="46.626615" />
                <line x1="104.230064" x2="104.230064" y1="2.966054" y2="32.542563" />
              </g>
              <g>
                <rect
                  x="72.218749"
                  ...
                  fill="transparent"
                />
                <line x1="95.457434" x2="95.457434" y1="66.853986" y2="96.430495" />
                <line x1="171.511314" x2="171.511314" y1="66.853986" y2="96.430495" />
                <line x1="72.218749" x2="194.75" y1="110.514547" y2="110.514547" />
                <text
                  transform="matrix(0.7042025923728943,0,0,0.7042025923728943,55.609912633895874,55.31411522626877) "
                  x="110.58531"
                  y="143.387146"
                  font-size="90"
                  stroke-width="1"
    ...
    >
      <slot name="header">
        <div class="mx-shortcuts-wrapper" v-if="!range && innerShortcutsSingle.length">
          <button
            type="button"
            class="mx-shortcuts"
            v-for="(range, index) in innerShortcutsSingle"
            :key="index"
            @click="selectRange(range)"
          >{{range.text}}</button>
        </div>
        <div class="mx-shortcuts-wrapper" v-if="range && innerShortcutsRange.length">
          <button
            type="button"
            class="mx-shortcuts"
            v-for="(range, index) in innerShortcutsRange"
            :key="index"
            @click="selectRange(range)"
          >{{range.text}}</button>
        </div>
...
export default {
  ...
  computed: {
    ...
    innerShortcutsSingle() {
      if (Array.isArray(this.shortcutsSingle)) {
        return this.shortcutsSingle
      }
      if (this.shortcutsSingle === false || this.range) {
        return []
      }

      const pickers = this.t('pickersSingle')
      const arr = [
        {
          text: pickers[0],
          onClick(self) {
    ...
    innerShortcutsRange() {
      if (Array.isArray(this.shortcutsRange)) {
        return this.shortcutsRange
      }
      if (this.shortcutsRange === false || !this.range) {
        return []
      }
      const pickers = this.t('pickersRange')
      const arr = [
        {
          text: pickers[0],
          onClick(self) {
  ...
  methods: {
    ...
    selectRange(range) {
      if (typeof range.onClick === 'function') {
        const onc = range.onClick(this)
        if (this.confirm && !confirm) {
        } else {
          this.updateDate(true)
          if (onc !== false && this.closeOnShortcuts) {
            this.closePopup()
          }
        }
      } else {
